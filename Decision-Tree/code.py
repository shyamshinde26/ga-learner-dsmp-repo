...
X=data.copy()
X.drop(columns=['customer.id','paid.back.loan'],axis=1,inplace=True)
...
print(fully_paid)
plt.bar(fully_paid[0],fully_paid[1])
plt.show()
# Code ends here
...
# Code starts here
X_train['int.rate']=X_train['int.rate'].apply(lambda x: x.replace('%','')).astype(float)
X_train['int.rate']=X_train['int.rate']/100
X_test['int.rate']=X_test['int.rate'].apply(lambda x: x.replace('%','')).astype(float)
X_test['int.rate']=X_test['int.rate']/100
print(X_train.info())
num_df=X_train.select_dtypes(include=[float,int])
